...
export default function App() {
  const [randomizedOrder] = useState(() => createShuffledOrder(cards.length));
  ...
  const handleNextCard = () => {
    let nextRandomIndex = Math.floor(Math.random() * cards.length);

    if (cards.length > 1) {
      while (
        randomizedOrder[nextRandomIndex] === randomizedOrder[currentIndex]
      ) {
        nextRandomIndex = Math.floor(Math.random() * cards.length);
      }
    }
    ...
    <main className="app">
      <section className="hero">
        ...
        <p className="description">
          ...
          use the next button to jump to another random card.
        </p>
